...
# time_dat = data.frame(year = rep(2006:2100,each=12),
#                      month = rep(1:12,95),
#                      ts.point = 1:(12*95))  ## future projection

gls.dat = data.frame(lon = ca_elevation[,1], lat = ca_elevation[,2],
                     elevation = ca_elevation[,3],
                     tas = tas_hist_ca,
...
autoimage(ca_elevation$longitude,ca_elevation$latitude,
          ...
          #breaks = seq(-5,30,by=0.01),
          #axis.args = list(at = seq(-5,30,by=5),labels = seq(-5,30,by=5)),
          #legend.axis.args = list(at = seq(-5,30,by=5),labels = seq(-5,30,by=5), las = 2),
          zlim = c(-6,31),
          #col=rev(cmap),
...
cmap <- rev(colorspace::sequential_hcl(n = 100, palette = "Viridis"))
par(oma = c(0, 0, 3, 0), mar = c(2, 1, 1, 1))
autoimage(coords_ca$longitude,coords_ca$latitude,
          ...
          #text.args = list(cex=2),
          legend.axis.args = list(cex.axis=1),
          #breaks = seq(-300,0,by=0.1),
          #axis.args = list(at = seq(300,0,length=8),labels = seq(100,0,length=7)),
          #legend.axis.args = list(at = seq(-300,0,by=50),labels = seq(300,0,by=-50), las = 2),
          zlim = c(0,200),
          col=cmap,
          size = c(1, 4), lratio = 0.35,
          legend = "vertical")
#legend.scale(c(-10,35), horizontal = FALSE)
...
for(j in 1:n.season)
{
  ...
  for(i in 1:n.loc)
  {
    ...
    d.resids[[i]] = density(residuals(lm.fit))
    norm_test[i,j] = shapiro.test(residuals(lm.fit))$p.value
  }
...
lm_acf = lm_pacf = matrix(NA, nrow = n.loc, ncol = n.season)
dw_p = matrix(NA, nrow = n.loc, ncol = n.season)
resid_mat = array(0, dim = c(n.loc, n.season, n.time))
acf_sig = qnorm(1 - 0.05 / 2) / sqrt(n.time)
...
for(i in 1:n.loc)
{
  for(j in 1:n.season)
  {
    ...
    dw_p[i,j] = durbinWatsonTest.glm(glm.fit)$p
