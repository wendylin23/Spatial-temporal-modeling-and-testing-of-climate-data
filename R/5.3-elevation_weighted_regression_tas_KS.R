...
#user = Sys.getenv("id")
#ks_elevation = elevation(longitude = coords_ks[,1],
#                          latitude = coords_ks[,2],elevation_model = "srtm1",
#                          username = user, verbose = FALSE)
tas_hist_ks_all = list.files(path = tas_path,pattern = "^tas_ks_tas.hist")
tas_rcp_ks_all = list.files(path = tas_path,pattern = "^tas_ks_tas.rcp85")
#pr_hist_ks_all = list.files(path = pr_path,pattern = "^pr_ks_pr.hist")
### example analysis for KS
tas_hist_ks = readRDS(file = paste0(tas_path,tas_hist_ks_all[1]))
tas_rcp_ks = readRDS(file = paste0(tas_path,tas_rcp_ks_all[1]))
...
## slopes (L2) vs. distance
par(mfrow = c(2,2),oma = c(0, 0, 3, 0), mar = c(2, 1, 1, 1))
slope_dist = array(0, dim = c(n.season, n.loc, n.loc))
for(t in 1:n.season)
{
  slope_dist[t,,] = (outer(beta_est[,t],beta_est[,t],"-"))^2
  tmp.slope = as.vector(slope_dist[t,,])
  vec.dist = as.vector(distance)
  dist.seq = seq(0, max(vec.dist), by=0.1)
  tmp.dat = data.frame(tmp.slope = tmp.slope, vec.dist = vec.dist)
  b_dist_lm = lm(tmp.slope ~ -1 + vec.dist,data = tmp.dat)
  b_dist_loess = loess(tmp.slope ~ -1 + vec.dist,data = tmp.dat)
  plot(tmp.dat$vec.dist, tmp.dat$tmp.slope,pch=".", cex=0.8, 
       main = season.var[t], xlab = "distance",ylab = "log(cor)")
  abline(b_dist_lm, col="red")
  lines(dist.seq, predict(b_dist_loess, newdata = data.frame(vec.dist = dist.seq)),col="blue")
}
...
lm_dat = data.frame(time.pts = gls.tas.summary$time.pts[-1],
                    winter = gls.tas.summary$winter[-1],
                    spring = gls.tas.summary$spring[-1],
                    summer = gls.tas.summary$summer[-1],
                    fall.b = gls.tas.summary$fall[-1],
                    fall.f = gls.tas.summary$fall[-nrow(gls.tas.summary)],
                    elevation = gls.tas.summary$elevation[-1])
m1 = lm(summer ~ time.pts, data = gls.tas.summary)
m2.1 = lm(winter ~ time.pts + elevation + fall.f, data = lm_dat)
m2.2 = lm(spring ~ time.pts + elevation + winter, data = lm_dat)
m2.3 = lm(summer ~ time.pts + elevation + spring, data = lm_dat)
m2.4 = lm(fall.b ~ time.pts + elevation+ summer, data = lm_dat)
...
for(i in 1:n.fold)
{
  for(j in 1:length(d_bis))
  {
    ...
    if(j==1)
    {
      ...
      fit_t2 = varw_fixed(tas_exp,p=0,xt = x_mult,w = w_t,fixed = fixed_beta2) ## no elevation
...
plot(d_bis[-1], apply(aic.bisquare.w2,2,mean)[-1], pch=16, axes=FALSE, 
     ylim=range(apply(aic.bisquare.w2,2,mean)[-1]), xlab="", ylab="", 
     type="b",col="black", main="")
axis(2, ylim=range(apply(aic.bisquare.w2,2,mean)[-1]),col="black",las=1)  ## las=1 makes horizontal labels
mtext("AIC",side=2,line=4)
box()
par(new=TRUE)
plot(d_bis[-1], apply(pred.err,1,mean)[-1], pch=15,
     xlab="", ylab="", ylim=range(apply(pred.err,1,mean)[-1]), 
     axes=FALSE, type="b", col="red")
mtext("Cross-validation MSE",side=4,col="red",line=4) 
axis(4, ylim=range(apply(pred.err,1,mean)[-1]), col="red",col.axis="red",las=1)
axis(1,pretty(range(d_bis[-1]),10))
mtext("threshold",side=1,col="black",line=2.5)  
...
## Bisquare kernel d[16]=1.6
source(paste0(code_path,"gwr.R"))
fixed_beta2 = rbind(matrix(1,nrow = 3,ncol = 4),diag(1,4))
fixed_beta3 = rbind(matrix(1,nrow = 3,ncol = 4),diag(0,4))
w1 = gwr.bisquare(distance^2,6)
w2 = gwr.lm(distance, 1.5)
w3 = gwr.exp(distance, 6)
w4 = gwr.gauss(distance^2,6)
w5 = gwr.inv(distance)
...
# ks_tas_coef = list(ks_tas_nw = ks_tas_nw,
#                    ks_tas_w = fit_m3_2,
#                    ks_tas_pre = pred_m3_2)
# save(ks_tas_coef,file = "/Users/wenyilin/Dropbox/UCSD/Thesis/3.Precipitation/Code/results/summary_results_20211101/ks_tas_coef.rdata")


## Prediction with future data
rcp_pred = pred.dat(x_rcp, fit_m3_2$coef)
...
## Test with other weight matrices
fit_m3_1 = varw_fixed(tas_exp,p=0,xt = x_mult,w = w1,fixed = fixed_beta2)
fit_m3_1s = varw_fixed(tas_exp,p=0,xt = x_mult,w = w1,fixed = fixed_beta3)
fit_m3_2 = varw_fixed(tas_exp,p=0,xt = x_mult,w = w2,fixed = fixed_beta2) ## selected
fit_m3_2s = varw_fixed(tas_exp,p=0,xt = x_mult,w = w2,fixed = fixed_beta3)
fit_m3_3 = varw_fixed(tas_exp,p=0,xt = x_mult,w = w3,fixed = fixed_beta2)
fit_m3_3s = varw_fixed(tas_exp,p=0,xt = x_mult,w = w3,fixed = fixed_beta3)
fit_m3_4 = varw_fixed(tas_exp,p=0,xt = x_mult,w = w4,fixed = fixed_beta2)
fit_m3_4s = varw_fixed(tas_exp,p=0,xt = x_mult,w = w4,fixed = fixed_beta3)
fit_m3_5 = varw_fixed(tas_exp,p=0,xt = x_mult,w = w5,fixed = fixed_beta2)

## summary for varying kernels
...
# for(i in 1:n.season)
# {
#   tmp.z = cbind(w_z_score[,i],z_score[,i])
#   hist(tmp.z[,1],xlab = "Z-score",main = paste0(season.var[i]," slope z scores"), breaks = 30, col=col1,xlim = range(tmp.z))
#   #hist(z_score$slope_m1[,2],col=rgb(0,1,1,1/4), add = T, breaks =30)
#   hist(tmp.z[,2],col=col2, add = T, breaks =30)
#   #hist(z_score$slope_m4[,2],col=rgb(1,1,0,1/4), add = T, breaks =30)
#   #hist(tmp.z[,3],col=col3, add = T, breaks =30)
#   if(i==n.season)
#     legend("topright",legend = c("GWR","MR"),
#            fill = c(col1,col2),
#            bty = 'n',cex = 0.75,
#            border = NA)
# }
