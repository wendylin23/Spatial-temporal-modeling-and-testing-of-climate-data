...
load(paste0(res_path,"ks_tas_cv.rdata"))
load(paste0(res_path,"ks_tas_aic_w2.rdata"))
...
N=1000
lon = unique(ks_elevation$longitude)
lat = unique(ks_elevation$latitude)
n = dim(ks_tas_coef$ks_tas_w$residuals)[1]

# Temperature data
for(s in 1:n.season)
{
  beta.long = data.frame(lon = ks_elevation$longitude, 
                         lat = ks_elevation$latitude,
                         b.gls = ks_tas_coef$ks_tas_nw$beta_est[,s],
                         b.gwr = ks_tas_coef$ks_tas_w$coef[2,s,]
  )
  se.long = data.frame(lon = ks_elevation$longitude, 
                       lat = ks_elevation$latitude,
                       b.gls = ks_tas_coef$ks_tas_nw$beta_se[,s],
                       b.gwr = ks_tas_coef$ks_tas_w$se.coef[2,s,]
  )
  ...
  for(i in 1:n)
  {
    long.t = data.frame(lon = ks_elevation$longitude, 
                        lat = ks_elevation$latitude,
                        #R.gls = ks_tas_coef$ks_tas_nw$resids[,i,2],
                        R.gls = ks_tas_coef$ks_tas_nw$resids[,i,3],
                        R.gwr = ks_tas_coef$ks_tas_w$residuals[i,3,])
  ...
  autoimage(ks_elevation$longitude,ks_elevation$latitude,
            ks_tas_coef$ks_tas_nw$beta_est,
  ...
  c95 = c05 = rep(0,dim(R.gls.tilde)[3])
  ## the jth order statistic from a uniform(0,1) sample has a beta(j,n-j+1) distribution (Casella & Berger, 2002)
  for(i in 1:dim(R.gls.tilde)[3]){
    c95[i] <- qbeta(0.95,i,dim(R.gls.tilde)[3]-i+1)
    c05[i] <- qbeta(0.05,i,dim(R.gls.tilde)[3]-i+1)
  }
  ...
  for(i in 1:length(lon))
  {
    for(j in 1:length(lat))
    {
      d.resids = density(R.gwr.tilde[i,j,])
