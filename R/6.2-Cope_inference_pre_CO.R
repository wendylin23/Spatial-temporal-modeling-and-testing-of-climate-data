...
n = dim(gwgr.pre.co$res.gr$dat_resid)[1]
...
for(s in 1:n.season)
{
  beta.long = data.frame(lon = co_elevation$longitude, 
                         ...
                         b.gls = gwgr.pre.co$res.gr$gwgr.slope[,s],
                         b.gwr = gwgr.pre.co$res.gwgr$gwgr.slope[,s]
  )
  se.long = data.frame(lon = co_elevation$longitude, 
                       lat = co_elevation$latitude,
                       b.gls = gwgr.pre.co$res.gwgr$gwgr.slope.se[,s],
                       b.gwr = gwgr.pre.co$res.gwgr$gwgr.slope.se[,s]
  )
  ...
  for(i in 1:n)
  {
    long.t = data.frame(lon = co_elevation$longitude, 
                        ...
                        R.gls = gwgr.pre.co$res.gr$dat_resid[i,s,],
                        R.gwr = gwgr.pre.co$res.gwgr$dat_resid[i,s,])
  ...
  par(oma = c(0, 0, 3, 0), mar = c(2, 1, 1, 1))
  autoimage(co_elevation$longitude,co_elevation$latitude,
            gwgr.pre.co$res.gwgr$gwgr.slope,
            # interp.args = list(no.X = 200, no.Y = 200),
            map = "county",ylab = "Latitude",xlab = "Longitude",
            main = season.var,
            outer.title = "Estimated slopes (mm/month/10 years) of four seasons",
