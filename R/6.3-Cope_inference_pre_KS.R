...
n = dim(gwgr.pre.ks$res.gr$dat_pred)[1]
...
for(s in 1:n.season)
{
  beta.long = data.frame(lon = ks_elevation$longitude, 
                         ...
                         b.gls = gwgr.pre.ks$res.gr$gwgr.slope[,s],
                         b.gwr = gwgr.pre.ks$res.gwgr$gwgr.slope[,s]
  )
  se.long = data.frame(lon = ks_elevation$longitude, 
                       lat = ks_elevation$latitude,
                       b.gls = gwgr.pre.ks$res.gwgr$gwgr.slope.se[,s],
                       b.gwr = gwgr.pre.ks$res.gwgr$gwgr.slope.se[,s]
  )
  ...
  for(i in 1:n)
  {
    long.t = data.frame(lon = ks_elevation$longitude, 
                        ...
                        R.gls = gwgr.pre.ks$res.gr$dat_resid[i,s,],
                        R.gwr = gwgr.pre.ks$res.gwgr$dat_resid[i,s,])
